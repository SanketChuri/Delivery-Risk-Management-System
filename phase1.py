...
class AlertConfig: 
    # Thresholds for classifying risk levels and urgency
...
def _fallback_synthetic_telemetry(df: pd.DataFrame, region: str = 'uk') -> pd.DataFrame:
    # Generate synthetic driver locations based on driver_id for fallback when telemetry is missing.
    out = df.copy()
...
def attach_driver_telemetry(
    # Attach driver locations to the DataFrame.
    df: pd.DataFrame,
